import { FC, createContext, ReactNode, useContext, useState, useMemo } from 'react'
...
export const AppContextProvider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const value = useMemo(() => ({ ready, setReady, securityConfig, setSecurityConfig, authConfig, setAuthConfig }), [ready, setReady, securityConfig, setSecurityConfig, authConfig, setAuthConfig]);
...
export function useInitAppContext () {
  ...
  configuration()
    .then((response) => {
      if (!response.data || !response.data.security || !response.data.oidc) {
        throw new Error("Got no configuration");
      }
