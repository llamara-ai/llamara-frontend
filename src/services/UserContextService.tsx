import { FC, createContext, ReactNode, useContext, useMemo, useState, useEffect } from 'react'
...
export const UserContextProvider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const value = useMemo(() => ({ loading, setLoading, ready, setReady, user, setUser }), [loading, setLoading, ready, setReady, user, setUser]);
...
export const useSetupUserContext = () => {
  ...
  useEffect(() => {
    ...
    login({
      ...
      if (!response.data) {
        throw new Error("Got no user info");
      }
