...
interface UseGetHistoryApiResponse {
    fetchHistory: ({sessionId} : FetchHistoryProps) => Promise<ChatMessageRecord[]>;
    loading: boolean;
    error: string | null;
}
...
export default function useGetHistoryApi() : UseGetHistoryApiResponse {
    ...
    const fetchHistory = async ({sessionId} : FetchHistoryProps): Promise<ChatMessageRecord[]>  => {
            ...
            try {
                ...
                if(response.data) {
                    ...
                    toast({
                        ...
                        description: "The given session id is invalid. Try again, or create new session.",
                    });
