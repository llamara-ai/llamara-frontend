...
export function SidebarModelSelector({models : modelsInput, selectedModel : selectedModelInput, setActiveModel}: Readonly<SidebarModelSelectorProps>) {
  ...
  return (
    <SidebarMenu>
      ...
      <SidebarMenuItem className={`transition-all duration-500 ${open ? 'mt-20' : 'mt-10'}`}>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            {selectedModel &&
                <div className="flex aspect-square size-8 items-center justify-center rounded-lg bg-black text-white">
                    <img src={selectedModel.logo} alt={selectedModel.label} className="w-6 h-6 invert" />
                </div>
            }
            ...
            {models.map((model, index) => (
              <Tooltip key={model.uid} > 
                <TooltipTrigger asChild>
                  <DropdownMenuItem
                    key={model.uid}
                    onClick={() => onClickSelectModel(model)}
                    className="gap-2 p-2"
                  >
                    <div className="flex size-6 items-center justify-center rounded-sm bg-black">
                      <img src={model.logo} alt={model.label} className="w-5 h-5 invert" />
                    </div>
                    {model.label}
